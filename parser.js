...
const makeRecipe = (appData, t) => {
    ...
    const tables = [];
    // Collections...
    const { collections, joins = [], datatypes, menu, packages, public, private, popups, templates } = appData.parts;
    // Packages
    ...
    public.items.forEach(page => {
        recipe.public_zone.pages.forEach(p => {
            if (p.name === 'home_public') {
                const [original, title, description] = page.match(/\s*(\w+):\s*Jumbotron:\s*(.*)$/);
    ...
    private.items.forEach(page => {
        ...
        const [original, pageName, collectionName] = page.match(/(\w+):\s*CRUD:\s*(.*)$/);
    ...
    collections.items.forEach(c => {
                ...
                if (fkeys[tableName])
                    fkeys[tableName].forEach(item => {
                        if (item.fkey === f) {
                            ...
                            const extraField = objectReplace(t.fkeyDisplay, addLC({}, { FOREIGNTABLE: item.target }));
        ...
        tables.push(
            [`CREATE TABLE \`${newC.name}\`\n(\n  _id varchar(17) PRIMARY KEY,`]
                .concat(
                    newC.fields
                        .map(c => {
                            let type = types[c.name] || 'varchar(255)';
                            if (c.name && (c.name.match(/Id$/) || c.name === '_id')) type = 'varchar(17)';
                            const comment = c.title ? `COMMENT '${c.title}'` : '';
                            const pk = c.name === '_id' ? 'PRIMARY KEY' : '';
                            const notnull = c.required ? 'NOT NULL' : '';
                            return `  \`${c.name}\` ${type} ${pk} ${notnull} ${comment}`;
                        })
                        .join(',\n'),
                    [');']
                )
        ...
        // Now output the Foreign key joins...
        // join_collection, join_fields[]
        newC.fields.map(c => {
            if (c.join_collection)
                tables.push(
                    `ALTER TABLE \`${newC.name}\` ADD FOREIGN KEY (\`${c.name}\`) REFERENCES \`${
                        c.join_collection
                    }\` (\`_id\`);`
                );
        });
    ...
    return [recipe, tables.map(item => item).join('\n\n\n')];
};
...
const parse = data => {
    ...
    errorData.errors.forEach(error => {
        ...
        errorData.summary[code] = errorData.summary[code] ? errorData.summary[code] + 1 : 1;
