...
Meteor.publish("comment_list", function() {
	if(Users.isInRoles(this.userId, ["admin","viewer","editor"])) {
		return Comments.publishJoinedCursors(Comments.find({}, {sort:{name:1}}));
	}
	return this.ready();
});
...
Meteor.publish("comment_empty", function() {
	if(Users.isInRoles(this.userId, ["admin","viewer","editor"])) {
		return Comments.publishJoinedCursors(Comments.find({_id:null}, {}));
	}
	return this.ready();
});
...
Meteor.publish("comment_details", function(commentId) {
	if(Users.isInRoles(this.userId, ["admin","viewer","editor"])) {
		return Comments.publishJoinedCursors(Comments.find({_id:commentId}, {}));
	}
	return this.ready();
});
...
Meteor.publish("comment_list_paged", function(extraOptions) {
	...
	if(Users.isInRoles(this.userId, ["admin","viewer","editor"])) {
		return Comments.publishJoinedCursors(Comments.find(databaseUtils.extendFilter({}, extraOptions), databaseUtils.extendOptions({sort:{name:1}}, extraOptions)));
	}
	return this.ready();
});
