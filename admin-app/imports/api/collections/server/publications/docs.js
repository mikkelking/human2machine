...
Meteor.publish("doc_list", function() {
	if(Users.isInRoles(this.userId, ["admin","viewer","editor"])) {
		return Docs.publishJoinedCursors(Docs.find({}, {sort:{name:1}}));
	}
	return this.ready();
});
...
Meteor.publish("doc_empty", function() {
	if(Users.isInRoles(this.userId, ["admin","viewer","editor"])) {
		return Docs.publishJoinedCursors(Docs.find({_id:null}, {}));
	}
	return this.ready();
});
...
Meteor.publish("doc_details", function(docId) {
	if(Users.isInRoles(this.userId, ["admin","viewer","editor"])) {
		return Docs.publishJoinedCursors(Docs.find({_id:docId}, {}));
	}
	return this.ready();
});
...
Meteor.publish("doc_list_paged", function(extraOptions) {
	...
	if(Users.isInRoles(this.userId, ["admin","viewer","editor"])) {
		return Docs.publishJoinedCursors(Docs.find(databaseUtils.extendFilter({}, extraOptions), databaseUtils.extendOptions({sort:{name:1}}, extraOptions)));
	}
	return this.ready();
});
