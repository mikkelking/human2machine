...
export class DocsInsertPage extends Component {
	constructor () {
		super();
			...
			return (
				<div>
					<div className="page-container container" id="content">
						...
						<DocsInsertPageInsertForm data={this.props.data} routeParams={this.props.routeParams} />
					</div>
				</div>
			);
		}
...
export const DocsInsertPageContainer = withTracker(function(props) {
	...
	if(isReady()) {
		...
		data = {
				...
				doc_empty: Docs.findOne({_id:null}, {})
			};
...
export class DocsInsertPageInsertForm extends Component {
	constructor () {
		super();

		this.state = {
			docsInsertPageInsertFormErrorMessage: "",
			docsInsertPageInsertFormInfoMessage: ""
		};
	...
	renderErrorMessage() {
		return(
			<div className="alert alert-warning">
				{this.state.docsInsertPageInsertFormErrorMessage}
			</div>
		);
	}
	...
	renderInfoMessage() {
		return(
			<div className="alert alert-success">
				{this.state.docsInsertPageInsertFormInfoMessage}
			</div>
		);
	}
	...
	onSubmit(e) {
		e.preventDefault();
		this.setState({ docsInsertPageInsertFormInfoMessage: "" });
		this.setState({ docsInsertPageInsertFormErrorMessage: "" });

		var self = this;
		var $form = $(e.target);

		function submitAction(result, msg) {
			var docsInsertPageInsertFormMode = "insert";
			if(!$("#docs-insert-page-insert-form").find("#form-cancel-button").length) {
				switch(docsInsertPageInsertFormMode) {
					case "insert": {
						$form[0].reset();
					}; break;
					...
					case "update": {
						...
						self.setState({ docsInsertPageInsertFormInfoMessage: message });
					}; break;
				}
			}
		...
		function errorAction(msg) {
			...
			self.setState({ docsInsertPageInsertFormErrorMessage: message });
		}
		...
		formUtils.validateForm(
			...
			function(values) {
				...
				Meteor.call("docsInsert", values, function(e, r) { if(e) errorAction(e); else submitAction(r); });
			}
		);
	...
	render() {
		let self = this;
		return (
			<div id="docs-insert-page-insert-form" className="">
				...
				<form role="form" onSubmit={this.onSubmit}>
					{this.state.docsInsertPageInsertFormErrorMessage ? this.renderErrorMessage() : null}
					{this.state.docsInsertPageInsertFormInfoMessage ? this.renderInfoMessage() : null}
					...
					<div className="form-group  field-markdown">
						...
						<div className="input-div">
							<input type="text" name="markdown" defaultValue="" className="form-control " required="required" />
							<span id="help-text" className="help-block" />
							<span id="error-text" className="help-block" />
						</div>
					</div>
					<div className="form-group  field-doctype">
						...
						<div className="input-div">
							<input type="text" name="doctype" defaultValue="" className="form-control " required="required" />
							<span id="help-text" className="help-block" />
							<span id="error-text" className="help-block" />
						</div>
					</div>
