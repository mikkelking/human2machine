...
Meteor.publish("star_list_paged", function(extraOptions) {
	...
	if(Users.isInRoles(this.userId, ["admin","viewer","editor"])) {
		return Stars.find(databaseUtils.extendFilter({}, extraOptions), databaseUtils.extendOptions({sort:{name:1}}, extraOptions));
	}
	return this.ready();
});
